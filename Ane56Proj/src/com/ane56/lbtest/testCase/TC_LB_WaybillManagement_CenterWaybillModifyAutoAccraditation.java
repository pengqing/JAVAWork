...
/**
 * 加盟网点中心运单修改审批(自动审批)流程2详情：运单发放 -> 运单发放查询 -> 寄件网点录单 -> 核对运单详情 -> 寄件网点发件扫描 ->
 * 首分拨到件扫描 -> 寄件运单管理(修改目的网点) -> 目的网点运单调整确认 -> 寄件运单查询 -> 首分拨发件扫描 -> 目的分拨到件扫描 ->
 * 目的分拨发件扫描 -> 目的网点到件扫描 -> 目的网点派件扫描 -> 目的网点签收扫描
 * 
 * @author WangHui
 * @version 1.0
...
public class TC_LB_WaybillManagement_CenterWaybillModifyAutoAccraditation extends BasePage
{
	...
	/*
	 * 重写初始化内容,将"billNoProvider.txt"中的第17条数据赋值给运单号
	 */
	@BeforeMethod
	public void setUp()
	{
	 ...
	 * 运单发放
	 */
	@Test(dataProvider = "DataProvider", dataProviderClass = DataProviders.class, description = "加盟网点中心运单修改审批(自动审批)-运单发放")
	...
	{
		logger.info("加盟网点中心运单修改审批(自动审批):运单发放Start...");
		driver.get(baseUrl);
		...
		logger.info("加盟网点中心运单修改审批(自动审批):运单发放Finish...");
	}

	/*
	 * 寄件网点录单
	 */
	@Test(dependsOnMethods = "issueWaybill", dataProvider = "DataProvider", dataProviderClass = DataProviders.class, description = "加盟网点中心运单修改审批(自动审批)-寄件网点录单及校验")
	public void wpointRecordWaybill(String userName, String password, String sendStatus) throws Exception

	{
		logger.info("加盟网点中心运单修改审批(自动审批):寄件网点录单Start...");
		...
		logger.info("加盟网点中心运单修改审批(自动审批):寄件网点录单Finish...");
	 ...
	 * 核对运单信息
	 */
	@Test(dependsOnMethods = "wpointRecordWaybill", priority = 1, dataProvider = "DataProvider", dataProviderClass = DataProviders.class, description = "加盟网点中心运单修改审批(自动审批)-核对运单信息")
	public void sendWaybillQueryAfterRecord(String userName, String password, String targetWpoint) throws Exception
	{
		logger.info("加盟网点中心运单修改审批(自动审批):核对运单信息Start...");
		driver.get(baseUrl);
		WaybillManagementMenus.login(userName, password);
		WaybillManagementMenus.sendWaybillManagement(startNo, targetWpoint, 0);
		logger.info("加盟网点中心运单修改审批(自动审批):核对运单信息Finish...");
	}

	/*
	 * 寄件网点发件扫描并进行发件扫描查询
	 */
	@Test(dependsOnMethods = "wpointRecordWaybill", priority = 2, dataProvider = "DataProvider", dataProviderClass = DataProviders.class, description = "加盟网点中心运单修改审批(自动审批)-寄件网点发件扫描并进行发件扫描查询")
	 ...
	 * 修改运单目的网点
	 */
	@Test(dependsOnMethods = "firstDistributionRecieveScan", dataProvider = "DataProvider", dataProviderClass = DataProviders.class, description = "加盟网点中心运单修改审批(自动审批)-修改运单目的网点")
	 ...
	 * 寄件运单查询
	 */
	@Test(dependsOnMethods = "wpointWaybillAdjuestConfirm", priority = 1, dataProvider = "DataProvider", dataProviderClass = DataProviders.class, description = "加盟网点中心运单修改审批(自动审批)-寄件运单查询")
	 ...
	 * 首分拨中心做发件扫描并进行发件扫描查询
	 */
	@Test(dependsOnMethods = "wpointWaybillAdjuestConfirm", priority = 2, dataProvider = "DataProvider", dataProviderClass = DataProviders.class, description = "加盟网点中心运单修改审批(自动审批)-首分拨中心做发件扫描并进行发件扫描查询")
	 ...
	 * 目的网点做签收扫描并进行签收扫描查询
	 */
	@Test(dependsOnMethods = "targetWpointDeliverScan", dataProvider = "DataProvider", dataProviderClass = DataProviders.class, description = "加盟网点中心运单修改审批(自动审批)-目的网点做签收扫描并进行签收扫描查询")
	public void targetWpointSignScan(String userName, String password, String recipient) throws InterruptedException
	{
		logger.info("加盟网点中心运单修改审批(自动审批):目的网点做签收扫描Start...");
		FileUtil.killProcess("SNE.exe");
		FileUtil.runFile("SNE.exe");
		String scriptPath = autoItScriptPath + MappingUtil.getClazzName() + "/TargetWpointSignScan.exe";
		FileUtil.runScript(scriptPath);
		logger.info("加盟网点中心运单修改审批(自动审批):目的网点做签收扫描Finish...");
		logger.info("加盟网点中心运单修改审批(自动审批):签收扫描查询Start...");
		driver.get(baseUrl);
		WaybillScanQueryMenus.login(userName, password);
		WaybillScanQueryMenus.signWaybillManagement(startNo, recipient);
		logger.info("加盟网点中心运单修改审批(自动审批):签收扫描查询Finish...");
	}
}
